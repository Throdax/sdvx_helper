...
def main():
    ...
    parser.add_argument('--pkl-file', '-f', default=None,
                        help='Migrate a single pkl file; type is auto-detected from the filename')
    ...
    if args.pkl_file:
        ...
        dispatch = {
            ...
            'musiclist.pkl':        (migrate_musiclist,         'musiclist.xml'),
            'rival_log.pkl':        (migrate_rival_log,         'rival_log.xml'),
            'title_conv_table.pkl': (migrate_title_conv_table,  'title_conv_table.xml'),
        }
        if name not in dispatch:
            print(f"ERROR: unknown pkl file '{name}' — expected one of: {', '.join(dispatch)}", file=sys.stderr)
            sys.exit(1)
        fn, xml_name = dispatch[name]
        fn(pkl_path=args.pkl_file, out_path=os.path.join(out_dir, xml_name))
        return

    # Batch mode: migrate all known pkl files from the current directory.
