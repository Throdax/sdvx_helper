 ...
 *
 * <p>
 * Google Drive redirects through a confirmation page for large files. This
 * client follows the redirect chain and handles the confirmation token
 * automatically, matching the behaviour of the Python implementation in
 * {@code sdvxh_classes.py} (the {@code dl_googledrive_csv()} method).
 * </p>
 *
 * @author Throdax
 * @since 2.0.0
...
public class GoogleDriveClient {
    ...
    /**
     * Constructs a Google Drive client with cookie support for the confirmation
     * flow.
     */
    public GoogleDriveClient() {
        this.http = HttpClient.newBuilder().connectTimeout(TIMEOUT).followRedirects(HttpClient.Redirect.NORMAL)
     ...
     *
     * @param fileId
     *            Google Drive file ID (from the share URL)
    ...
    /**
     * Downloads content from the given URL, handling the Google Drive virus-scan
