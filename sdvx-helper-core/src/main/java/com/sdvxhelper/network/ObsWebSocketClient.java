 ...
 *
 * <p>Wraps the {@code io.obs-websocket.community:client} library to provide a
 ...
 * and updating text source values.</p>
 *
 * <p>The underlying library is fully asynchronous; this wrapper bridges each
 * async callback into a blocking {@link CompletableFuture} with a
 * {@value #REQUEST_TIMEOUT_SECONDS}-second timeout so callers do not have to
 * manage threading themselves.</p>
 *
...
public class ObsWebSocketClient implements Closeable {
     ...
     * @param port     OBS WebSocket port (e.g. {@code 4455})
     * @param password OBS WebSocket password (empty string if authentication is disabled)
     */
    ...
    /**
     * Establishes a connection to the OBS WebSocket server and blocks until
     * the handshake completes or the connection times out.
     *
     ...
     * {@link BufferedImage}.
     *
     * @param sourceName the OBS source name to capture
     * @return the captured image
     * @throws IOException if the screenshot request fails or the image cannot be decoded
     */
    public BufferedImage captureSource(String sourceName) throws IOException {
        ...
        BufferedImage img = decodeBase64Image(
                resp.getMessageData().getResponseData().getImageData());
     ...
     *
     * @param base64Data Base64 string (may include the
     *                   {@code data:image/png;base64,} data-URL prefix)
     * @return the decoded {@link BufferedImage}
     * @throws IOException if decoding fails or the image data is invalid
     */
     ...
     *
     * <p>This operation requires two OBS requests: first
     * {@code GetSceneItemId} to resolve the numeric scene item ID from the
     * source name, then {@code SetSceneItemEnabled} to apply the visibility
     * change.</p>
     *
     * @param sceneName  the name of the scene containing the source
     * @param sourceName the name of the source to show or hide
     * @param visible    {@code true} to show the source, {@code false} to hide it
     * @throws IOException if either OBS request fails
     */
    public void setSourceVisible(String sceneName, String sourceName, boolean visible)
        ...
        if (!idResp.isSuccessful()) {
            throw new IOException("OBS GetSceneItemId failed for source '" + sourceName
                    + "' in scene '" + sceneName + "': "
     ...
     * @throws IOException if the OBS request fails
     */
     ...
     *
     * @param sourceName the name of the text input source
     * @param text       the new text value to display
     * @throws IOException if the OBS request fails
     */
     ...
     *
     * <p>The callback receives the raw event string for diagnostic or
     * custom-handling purposes. For typed event handling use
     * {@link OBSRemoteController.OBSRemoteControllerBuilder#registerEventListener} during
    ...
    public void onEvent(Consumer<String> eventHandler) {
        ...
        // events should rebuild the controller with the appropriate listener registered.
     ...
     *
     * @throws IOException if not connected to OBS
     */
     ...
     * @param requestName the OBS request name, used in error messages
     * @return the future's result
     * @throws IOException if the future completes exceptionally, times out, or
    ...
    private <T> T await(CompletableFuture<T> future, String requestName) throws IOException {
        try {
            ...
            throw new IOException("OBS request '" + requestName + "' timed out after "
                    + REQUEST_TIMEOUT_SECONDS + "s", e);
        } catch (ExecutionException e) {
