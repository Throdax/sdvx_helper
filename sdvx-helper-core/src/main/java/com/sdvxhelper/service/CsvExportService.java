 ...
 *
 * <p>
 * Replaces the Python {@code gen_best_csv()}, {@code gen_alllog_csv()}, and
 * {@code gen_playcount_csv()} methods in {@code SDVXLogger}.
 *
 * <p>
 * The alllog CSV is exported in Shift-JIS (to match the original Python
 * behaviour) while other CSVs use UTF-8. Both encodings are configurable via
 * the write methods.
 * </p>
 *
 * @author Throdax
...
public class CsvExportService {
    ...
    /**
     * Writes a best-scores CSV ({@code out/best_*.csv}) containing one row per
     * chart.
     *
     * <p>
     * Columns: {@code title,difficulty,lv,score,lamp,vf}
     * </p>
     *
     * @param bestList
     *            list of personal-best {@link MusicInfo} records (VF desc order)
     * @param outFile
     *            destination file
     * @throws IOException
    ...
    public void writeBestCsv(List<MusicInfo> bestList, File outFile) throws IOException {
        ...
        try (PrintWriter pw = new PrintWriter(outFile, StandardCharsets.UTF_8)) {
            ...
            for (MusicInfo m : bestList) {
                pw.printf("%s,%s,%s,%d,%s,%s%n", csvEsc(m.getTitle()), csvEsc(m.getDifficulty()), csvEsc(m.getLv()),
                        m.getBestScore(), csvEsc(m.getBestLamp()), ScoreFormatter.formatVf(m.getVf()));
            }
        }
    ...
    /**
     * Writes the full play-history CSV ({@code out/alllog_*.csv}) in Shift-JIS to
     * match the original Python output encoding.
     *
     * <p>
     * Columns: {@code title,difficulty,lv,score,lamp,vf,date}
     * </p>
     *
     * @param plays
     *            all play records (date ascending)
     * @param outFile
     *            destination file
     * @throws IOException
     *             if the file cannot be written
    ...
    public void writeAllLogCsv(List<OnePlayData> plays, File outFile) throws IOException {
        ...
        try (PrintWriter pw = new PrintWriter(outFile, SHIFT_JIS)) {
            ...
            for (OnePlayData p : plays) {
                pw.printf("%s,%s,%d,%s,%+d,%s%n", csvEsc(p.getTitle()), csvEsc(p.getDifficulty()), p.getCurScore(),
                        csvEsc(p.getLamp()), p.getDiff(), csvEsc(MusicInfo.marshalDate(p.getDate())));
            }
        }
    ...
    public void writePlayCountCsv(List<OnePlayData> plays, File outFile) throws IOException {
        ...
        for (OnePlayData p : plays) {
            String date = p.getDate() != null ? p.getDate().toLocalDate().toString() : "unknown";
    ...
    /**
     * Ensures the parent directory of the given file exists, creating it if
     * necessary.
     *
     * @param file
     *            the file whose parent directory should be checked
     * @throws IOException
