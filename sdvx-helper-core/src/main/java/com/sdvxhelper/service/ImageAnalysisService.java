...
/**
 * Analyses captured OBS frames to determine the current game state and song identity.
 *
 * <p>Decomposes the image-analysis portion of the Python {@code GenSummary} class
...
public class ImageAnalysisService {
    ...
    /**
     * Determines the current game state from a full-frame capture by testing
     * a set of known reference image crops.
     *
     * <p>This is a placeholder implementation; the real detection uses a region
     ...
     * pipeline should be populated in the concrete OBS-capture integration.</p>
     *
     ...
     * @param regionParams  map of parameter keys to numeric region values from params.json
     * @return detected {@link DetectMode}
     */
    public DetectMode detectMode(BufferedImage frame,
                                 Map<DetectMode, String> stateHashes,
    ...
    public String[] identifyJacket(BufferedImage frame, Rectangle jacketRegion, String difficulty) {
        try {
            BufferedImage jacketCrop = crop(frame,
                    jacketRegion.x, jacketRegion.y,
                    jacketRegion.width, jacketRegion.height);
     ...
     *
     * <p>The Python implementation sums the colour channels of a lamp-indicator
     * region and compares against pre-measured thresholds.  This method accepts
     * the pre-cropped lamp region image.</p>
     *
    ...
    public String detectLamp(BufferedImage lampRegionImage) {
        ...
        if (rAvg > LAMP_THRESHOLD && gAvg > LAMP_THRESHOLD && bAvg > LAMP_THRESHOLD) return "puc";
        if (rAvg > LAMP_THRESHOLD && gAvg > LAMP_THRESHOLD) return "uc";
        if (gAvg > LAMP_THRESHOLD) return "clear";
        if (rAvg > LAMP_THRESHOLD) return "hard";
        if (bAvg > LAMP_THRESHOLD) return "exh";
        return "failed";
    }
