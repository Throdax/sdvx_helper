...
/**
 * Integration-level tests for {@link ImageAnalysisService} using real
 * SDVX result-screen captures placed in {@code src/test/resources}.
 *
 * <p>The test fixture is {@code sdvx_666_NOV_hard_992_20250424_183843.png}
 * (1080 × 1920 px), a result screen for the song "666" played on NOVICE
 * difficulty with a HARD clear.  All crop coordinates are taken verbatim
 * from {@code resources/params.json}.</p>
 *
 * <p>Tests validate structural invariants (image and crop dimensions,
 * hash stability, channel statistics) rather than the current heuristic
...
class ImageAnalysisServiceTest {
    ...
    void resultScreenHasExpectedDimensions() {
        Assertions.assertEquals(RESULT_W, resultScreen.getWidth(),
                "Result screen width must be " + RESULT_W);
        Assertions.assertEquals(RESULT_H, resultScreen.getHeight(),
                "Result screen height must be " + RESULT_H);
    }
    ...
    void lampCropHasExpectedDimensions() {
        ...
        Assertions.assertEquals(LAMP_W, crop.getWidth(),
                "Lamp crop width must equal lamp_w from params.json");
        Assertions.assertEquals(LAMP_H, crop.getHeight(),
                "Lamp crop height must equal lamp_h from params.json");
    }
    ...
    void scoreLargeDigitCropHasExpectedDimensions() {
        ...
        Assertions.assertEquals(SCORE_D0_W, crop.getWidth(),
                "Large score digit crop width must match params.json");
        Assertions.assertEquals(SCORE_D0_H, crop.getHeight(),
                "Large score digit crop height must match params.json");
    }
    ...
    void scoreSmallDigitCropHasExpectedDimensions() {
        ...
        Assertions.assertEquals(SCORE_D4_W, crop.getWidth(),
                "Small score digit crop width must match params.json");
        Assertions.assertEquals(SCORE_D4_H, crop.getHeight(),
                "Small score digit crop height must match params.json");
    }
     ...
     * lamp indicator renders in warm orange/red tones.  We assert that the red channel
     * average is the dominant colour channel in the lamp region, which is a stable
     * property of the HARD lamp colour regardless of the exact threshold values used
     * by the classifier heuristic.
     */
    @Test
    ...
    void allParamsCropRegionsAreWithinResultScreenBounds() {
        int[][] regions = {
            { JACKET_X, JACKET_Y, JACKET_W, JACKET_H },
            { LAMP_X,   LAMP_Y,   LAMP_W,   LAMP_H   },
            { SCORE_D0_X, SCORE_D0_Y, SCORE_D0_W, SCORE_D0_H },
            { SCORE_D4_X, SCORE_D4_Y, SCORE_D4_W, SCORE_D4_H },
        };
        for (int[] r : regions) {
            int x = r[0], y = r[1], w = r[2], h = r[3];
            Assertions.assertTrue(x + w <= RESULT_W,
                    "Crop x+w=" + (x + w) + " exceeds result width " + RESULT_W);
            Assertions.assertTrue(y + h <= RESULT_H,
                    "Crop y+h=" + (y + h) + " exceeds result height " + RESULT_H);
        }
    }
    ...
    void jacketCropFromResultHashIsStableAndValidFormat() {
        ...
        Assertions.assertEquals(hash1, hash2,
                "Hashing the same crop twice must produce identical results");
        Assertions.assertEquals(16, hash1.length(),
                "Hash must be 16 hex characters");
        Assertions.assertTrue(hash1.matches("[0-9a-f]{16}"),
                "Hash must be lowercase hex: " + hash1);
    }
    ...
    void jacketFileHashIsStableAndValidFormat() throws IOException {
        ...
        Assertions.assertEquals(hash1, hash2);
        Assertions.assertEquals(16, hash1.length());
        Assertions.assertTrue(hash1.matches("[0-9a-f]{16}"), "Expected lowercase hex: " + hash1);
    }
    ...
    /**
     * Verifies that the two test fixture images produce different hashes, confirming
     * that the hasher actually discriminates between distinct images rather than
    ...
    void jacketFileHashDiffersFromJacketCropInResultScreen() throws IOException {
        ...
        // These are different images (different songs / different cropping) so their hashes must differ.
     ...
     *
     * <p>Digit template images ({@code result_score_l0-9.png} for the large
     * first-four digits and {@code result_score_s0-9.png} for the smaller last-four)
     * are loaded from {@code src/main/resources/images/} — exactly the same files
     * used by the runtime application.</p>
     *
     * <p>Digit regions are cropped using the coordinates from {@code params.json}:
     * <ul>
     *   <li>Digits 0–3 (large, 52 × 51 px): {@code result_score_large_0} … {@code result_score_large_3}</li>
     *   <li>Digits 4–7 (small, 32 × 31 px): {@code result_score_small_4} … {@code result_score_small_7}</li>
     * </ul>
     * Ground truth (9 928 527) confirmed with the Python reference implementation.</p>
     */
    @Test
    void scoreFromResultScreenIsDetectedCorrectly() throws IOException {
        ...
        for (int[] c : largeCrops) {
            ...
            Assertions.assertNotEquals('?', d,
                    "Large digit at x=" + c[0] + " could not be matched against templates");
            sb.append(d);
        }
        for (int[] c : smallCrops) {
            ...
            Assertions.assertNotEquals('?', d,
                    "Small digit at x=" + c[0] + " could not be matched against templates");
            sb.append(d);
        }

        int detectedScore = Integer.parseInt(sb.toString());
        Assertions.assertEquals(9_928_527, detectedScore,
                "Assembled score from 8 digit crops must match ground truth");
    }

    /**
     * Verifies that each of the 4 large-digit crops from the result screen is
     * individually recognisable (distance to best template ≤ {@code MATCH_THRESHOLD}).
     * This isolates individual crop quality from score-assembly concerns.
     */
    @Test
    ...
    /**
     * Verifies that passing all 8 digit crops in one list to
     * {@link ScoreDetector#detectScore(java.util.List)} succeeds when appropriate
     * templates are used.  Uses large templates for all 8 positions to exercise
     * the API; individual digit accuracy is covered by the per-digit tests above.
     */
    @Test
    void detectScoreApiReturnsNonNegativeForLargeTemplates() throws IOException {
        Map<Character, String> largeTemplates = loadDigitTemplates("result_score_l");
        ScoreDetector detector = new ScoreDetector(largeTemplates);

        java.util.List<BufferedImage> allCrops = new java.util.ArrayList<>();
        int[][] all = {
            {431, 1069, 52, 51}, {489, 1069, 52, 51},
            {547, 1069, 52, 51}, {605, 1069, 52, 51},
            {662, 1089, 32, 31}, {698, 1089, 32, 31},
            {734, 1089, 32, 31}, {770, 1089, 32, 31},
        };
        for (int[] c : all) {
            allCrops.add(resultScreen.getSubimage(c[0], c[1], c[2], c[3]));
        }

        int score = detector.detectScore(allCrops);
        Assertions.assertNotEquals(-1, score,
                "detectScore must not return -1 (recognition failure) for large-template matching");
        Assertions.assertTrue(score >= 0 && score <= 10_000_000,
                "Detected score must be in valid SDVX range [0, 10000000], was: " + score);
    }
    ...
    /**
     * Builds a digit-template hash map for {@link ScoreDetector} by loading the
     * ten digit PNGs ({@code prefix0.png} … {@code prefix9.png}) from the main
     * classpath resources ({@code src/main/resources/images/}).
     *
     * @param prefix image filename prefix, e.g. {@code "result_score_l"} or
     *               {@code "result_score_s"}
     * @return map from digit character ({@code '0'}–{@code '9'}) to its aHash string
